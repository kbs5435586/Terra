...
_bool CBuffer_Terrain::Picking_ToBuffer(_vec3* pOut, const CTransform* pTransformCom, const CPicking* pPickingCom)
{
	...
	for (_uint i = 0; i < m_vecVertex.size(); i += 3)
	{
		isPick = D3DXIntersectTri(&m_vecVertex[i], &m_vecVertex[i + 1],
			&m_vecVertex[i + 2], &vRayPivot, &vRay, &fU, &fV, &fDist);
		if (isPick)
		{

			_vec3	out = _vec3(vRayPivot + vRay * fDist);
			//out.x = _float((int)(out.x / m_fInterval) * m_fInterval + m_fInterval * 0.5f);
			//out.z = _float((int)(out.z / m_fInterval) * m_fInterval + m_fInterval * 0.5f);
			*pOut = out;
			break;
		}
	}
