...
HRESULT CRenderer::Ready_Renderer()
{
	...
	if (FAILED(m_pTarget_Manager->Add_Target(m_pGraphic_Device, L"Target_Effect", ViewPort.Width, ViewPort.Height, D3DFMT_A16B16G16R16F, D3DXCOLOR(0.f, 0.f, 0.f, 0.f))))
		return E_FAIL;
	if (FAILED(m_pTarget_Manager->Ready_Debug_Buffer(L"Target_Effect", 200.0f, 400.f, 200.f, 200.f)))
		return E_FAIL;
	if (FAILED(m_pTarget_Manager->Add_Target(m_pGraphic_Device, L"Target_Effect2", ViewPort.Width, ViewPort.Height, D3DFMT_A16B16G16R16F, D3DXCOLOR(0.f, 0.f, 0.f, 0.f))))
		return E_FAIL;
	if (FAILED(m_pTarget_Manager->Ready_Debug_Buffer(L"Target_Effect2", 200.0f, 600.f, 200.f, 200.f)))
		return E_FAIL;
	...
	if (FAILED(m_pTarget_Manager->Add_MRT(L"MRT_Effect", L"Target_Effect")))
		return E_FAIL;
	if (FAILED(m_pTarget_Manager->Add_MRT(L"MRT_Effect", L"Target_Effect2")))
		return E_FAIL;

	// For.Shader_LightAcc
	m_pShaderCom_LightAcc = CShader::Create(m_pGraphic_Device, L"../ShaderFiles/Shader_LightAcc.fx");
	if (nullptr == m_pShaderCom_LightAcc)
		return E_FAIL;

	// For.Shader_Blend
	m_pShaderCom_Blend = CShader::Create(m_pGraphic_Device, L"../ShaderFiles/Shader_Blend.fx");
...
void CRenderer::Render_Effect_Defered()
{
