#include "pch.h"
#include "Buffer_CubeWire.h"

CBuffer_CubeWire::CBuffer_CubeWire(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CVIBuffer(pGraphic_Device)
{
}

CBuffer_CubeWire::CBuffer_CubeWire(const CBuffer_CubeWire & rhs)
	: CVIBuffer(rhs)
{
}

HRESULT CBuffer_CubeWire::Ready_VIBuffer()
{
	vector<_vec3> vecV;

	m_iNumVertices = 8;
	m_iStride = sizeof(VTXCUBEWIR);
	m_dwFVF = D3DFVF_XYZ;
	...
	{
		vecV.push_back(pVertices[i].vPosition);
	}
...
void CBuffer_CubeWire::Render_VIBuffer()
{
...
_bool CBuffer_CubeWire::Picking_ToBuffer(_vec3 * pOut, const CTransform * pTransformCom, const CPicking * pPickingCom)
{
...
CBuffer_CubeWire * CBuffer_CubeWire::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CBuffer_CubeWire*	pInstance = new CBuffer_CubeWire(pGraphic_Device);

	if (FAILED(pInstance->Ready_VIBuffer()))
	{
		MessageBox(0, L"CBuffer_CubeWire Created Failed", L"System Error", MB_OK);
		Safe_Release(pInstance);
	}
	return pInstance;
}

CComponent * CBuffer_CubeWire::Clone_Component(void* pArg)
{
	return new CBuffer_CubeWire(*this);
}

void CBuffer_CubeWire::Free()
{
	CVIBuffer::Free();
