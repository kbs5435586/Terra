...
CUD_LightCarvarly::CUD_LightCarvarly(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CGameObject(pGraphic_Device)
{
}

CUD_LightCarvarly::CUD_LightCarvarly(const CUD_LightCarvarly & rhs)
	: CGameObject(rhs)
{
}

HRESULT CUD_LightCarvarly::Ready_Prototype()
{
	return S_OK;
}

HRESULT CUD_LightCarvarly::Ready_GameObject(void * pArg)
{
...
_int CUD_LightCarvarly::Update_GameObject(const _float & fTimeDelta)
{
	...
	{
		m_IsOnce = true;
		m_eCurState = CUD_LightCarvarly::STATE_SPON1;
	}
...
_int CUD_LightCarvarly::LastUpdate_GameObject(const _float & fTimeDelta)
{
	...
	if (m_pMeshCom->Get_EndLoop() && m_IsOnce)
	{
		//SPON
		{
			if (m_eCurState == CUD_LightCarvarly::STATE_SPON1)
				m_eCurState = CUD_LightCarvarly::STATE_SPON2;
			if (m_eCurState == CUD_LightCarvarly::STATE_SPON2)
				m_fOnIdleTime += fTimeDelta;
			if (m_fOnIdleTime >= 0.05f)
			{
				m_eCurState = CUD_LightCarvarly::STATE_IDLE;
				m_IsOnce = false;
				m_fOnIdleTime = 0.f;
			}
		}
...
void CUD_LightCarvarly::Render_GameObject()
{
	if (nullptr == m_pMeshCom ||
...
CUD_LightCarvarly * CUD_LightCarvarly::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CUD_LightCarvarly*	pInstance = new CUD_LightCarvarly(pGraphic_Device);
...
CGameObject * CUD_LightCarvarly::Clone_GameObject(void * pArg)
{
	CUD_LightCarvarly*	pInstance = new CUD_LightCarvarly(*this);
...
void CUD_LightCarvarly::Change_State(STATE eID)
{
	if (m_ePreState != eID)
	{
		switch (eID)
		{
		case CUD_LightCarvarly::STATE_SPON1:
			m_pMeshCom->Set_AnimationSet((_int)CUD_LightCarvarly::STATE_SPON1);
			break;
		case CUD_LightCarvarly::STATE_SPON2:
			m_pMeshCom->Set_AnimationSet((_int)CUD_LightCarvarly::STATE_SPON2);
			break;
		case CUD_LightCarvarly::STATE_IDLE:
			m_pMeshCom->Set_AnimationSet((_int)CUD_LightCarvarly::STATE_IDLE);
			break;
		}
...
HRESULT CUD_LightCarvarly::Ready_Component()
{
	CManagement*		pManagement = CManagement::GetInstance();
	if (nullptr == pManagement)
		return E_FAIL;
	...
	// For.Com_Mesh
	m_pMeshCom = (CDynamic_Mesh*)pManagement->Clone_Component(SCENE_LOGO, L"Component_Mesh_UD_LightCarvarly");
...
HRESULT CUD_LightCarvarly::SetUp_ConstantTable(LPD3DXEFFECT pEffect, const D3DXMESHCONTAINER_DERIVED * pMeshContainer, const _uint & iAttributeID)
{
