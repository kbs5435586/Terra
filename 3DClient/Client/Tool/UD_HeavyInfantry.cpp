...
CUD_HeavyInfantry::CUD_HeavyInfantry(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CGameObject(pGraphic_Device)
{
}

CUD_HeavyInfantry::CUD_HeavyInfantry(const CUD_HeavyInfantry & rhs)
	: CGameObject(rhs)
{
}

HRESULT CUD_HeavyInfantry::Ready_Prototype()
{
	return S_OK;
}

HRESULT CUD_HeavyInfantry::Ready_GameObject(void * pArg)
{
...
_int CUD_HeavyInfantry::Update_GameObject(const _float & fTimeDelta)
{
	...
	{
		m_IsOnce = true;
		m_eCurState = CUD_HeavyInfantry::STATE_SPON1;
	}
...
_int CUD_HeavyInfantry::LastUpdate_GameObject(const _float & fTimeDelta)
{
	...
	if (m_pMeshCom->Get_EndLoop() && m_IsOnce)
	{
		//SPON
		{
			if (m_eCurState == CUD_HeavyInfantry::STATE_SPON1)
				m_eCurState = CUD_HeavyInfantry::STATE_SPON2;
			if (m_eCurState == CUD_HeavyInfantry::STATE_SPON2)
				m_fOnIdleTime += fTimeDelta;
			if (m_fOnIdleTime >= 0.05f)
			{
				m_eCurState = CUD_HeavyInfantry::STATE_IDLE;
				m_IsOnce = false;
				m_fOnIdleTime = 0.f;
			}
		}
...
void CUD_HeavyInfantry::Render_GameObject()
{
	if (nullptr == m_pMeshCom ||
...
CUD_HeavyInfantry * CUD_HeavyInfantry::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CUD_HeavyInfantry*	pInstance = new CUD_HeavyInfantry(pGraphic_Device);
...
CGameObject * CUD_HeavyInfantry::Clone_GameObject(void * pArg)
{
	CUD_HeavyInfantry*	pInstance = new CUD_HeavyInfantry(*this);
...
void CUD_HeavyInfantry::Change_State(STATE eID)
{
	if (m_ePreState != eID)
	{
		switch (eID)
		{
		case CUD_HeavyInfantry::STATE_SPON1:
			m_pMeshCom->Set_AnimationSet((_int)CUD_HeavyInfantry::STATE_SPON1);
			break;
		case CUD_HeavyInfantry::STATE_SPON2:
			m_pMeshCom->Set_AnimationSet((_int)CUD_HeavyInfantry::STATE_SPON2);
			break;
		case CUD_HeavyInfantry::STATE_IDLE:
			m_pMeshCom->Set_AnimationSet((_int)CUD_HeavyInfantry::STATE_IDLE);
			break;
		}
...
HRESULT CUD_HeavyInfantry::Ready_Component()
{
	CManagement*		pManagement = CManagement::GetInstance();
	if (nullptr == pManagement)
		return E_FAIL;
	...
	// For.Com_Mesh
	m_pMeshCom = (CDynamic_Mesh*)pManagement->Clone_Component(SCENE_LOGO, L"Component_Mesh_UD_HeavyInfantry");
...
HRESULT CUD_HeavyInfantry::SetUp_ConstantTable(LPD3DXEFFECT pEffect, const D3DXMESHCONTAINER_DERIVED * pMeshContainer, const _uint & iAttributeID)
{
