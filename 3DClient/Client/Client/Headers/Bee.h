...
{
public:
	const _float ATTACK_RANGE = 0.5f;
public:
	enum STATE_ {STATE_IDLE, STATE_WALK, STATE_RUN, STATE_ATT, STATE_DEATH,STATE_DEATH_WAIT, STATE_END};
	enum ROTATE_DIR { DIR_LEFT, DIR_RIGHT, DIR_END };
private:
	CBee(LPDIRECT3DDEVICE9 pGraphic_Device);
	CBee(const CBee& rhs);
...
public:
	static	CBee*			Create(LPDIRECT3DDEVICE9 pGraphic_Device);
