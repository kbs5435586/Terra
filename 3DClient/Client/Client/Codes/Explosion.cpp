...
CExplosion::CExplosion(LPDIRECT3DDEVICE9 pGraphic_Device, _uint iNumParticle)
...
CExplosion::CExplosion(CExplosion& rhs)
	: CParticleSystem(rhs)
...
HRESULT CExplosion::Ready_Prototype()
{
	return S_OK;
}
...
void CExplosion::preRender()
{
	CParticleSystem::Pre_Render();
	m_pGraphic_Device->SetRenderState(D3DRS_ZWRITEENABLE, false);
}

void CExplosion::postRender()
{
	CParticleSystem::Post_Render();
	m_pGraphic_Device->SetRenderState(D3DRS_ZWRITEENABLE, true);
}

void CExplosion::Reset_Particle(PSATTRIBUTE* pAttribute)
{
	static int i = 0;
...
HRESULT CExplosion::Ready_Component(void* pArg)
{
	...
	m_pTextureCom = (CTexture*)pManagement->Clone_Component(SCENE_STATIC, L"Component_Texture_Explosion");
...
CExplosion* CExplosion::Create(PDIRECT3DDEVICE9 pGrapic_Device, _uint iNumParticle)
{
	CExplosion* pInstance = new CExplosion(pGrapic_Device, iNumParticle);
	pInstance->Init();
...
CGameObject* CExplosion::Clone_GameObject(void* pArg)
{
