#include "stdafx.h"
#include "..\Headers\FireSpark.h"
#include "Management.h"


CFireSpark::CFireSpark(LPDIRECT3DDEVICE9 pGraphic_Device, _uint iNumParticle)
...
CFireSpark::CFireSpark(CFireSpark& rhs)
	: CParticleSystem(rhs)
	, m_iNumParticle(rhs.m_iNumParticle)
{
	//m_iParticleSize = 7.f;
	m_iParticleSize = 400.f;
...
HRESULT CFireSpark::Ready_Prototype()
{
	return S_OK;
}

HRESULT CFireSpark::Ready_GameObject(void* pArg)
{
	...
	_vec3 vPos = *m_pParentTransform->Get_StateInfo(STATE_POSITION);
	m_pTransformCom->Set_StateInfo(STATE_POSITION, &vPos);
	return S_OK;
}

_int CFireSpark::Update_GameObject(const _float& fTimeDelta)
{
...
_int CFireSpark::LastUpdate_GameObject(const _float& fTimeDelta)
{
...
void CFireSpark::Render_GameObject()
{
	CParticleSystem::Render_GameObject();
}

void CFireSpark::preRender()
{
	CParticleSystem::Pre_Render();
	m_pGraphic_Device->SetRenderState(D3DRS_ZWRITEENABLE, false);
}

void CFireSpark::postRender()
{
	CParticleSystem::Post_Render();
	m_pGraphic_Device->SetRenderState(D3DRS_ZWRITEENABLE, true);
}

void CFireSpark::Reset_Particle(PSATTRIBUTE* pAttribute)
{
	static int i = 0;
	...
	pAttribute->vVelocity = /*vLook * 7 + vRight * GetRandom_Float(-3.f, 3.f) + */vUp * GetRandom_Float(-2.f, 0.f);

	pAttribute->vAcceleration.x = 0.f;
	pAttribute->vAcceleration.y = -100.f;
...
HRESULT CFireSpark::Ready_Component(void* pArg)
{
...
CFireSpark* CFireSpark::Create(PDIRECT3DDEVICE9 pGrapic_Device, _uint iNumParticle)
{
	CFireSpark* pInstance = new CFireSpark(pGrapic_Device, iNumParticle);
	pInstance->Init();
...
CGameObject* CFireSpark::Clone_GameObject(void* pArg)
{
	CFireSpark* pInstance = new CFireSpark(*this);
