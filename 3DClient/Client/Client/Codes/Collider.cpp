...
HRESULT CCollider::Ready_Clone_Collider(void * pArg)
{
	...
	{
		D3DXVec3TransformCoord((_vec3*)((_byte*)pVertices + (i * iVertexSize)), 
			(_vec3*)((_byte*)pVertices + (i * iVertexSize)), &pColliderInfo->matLocalTransform);
	}
...
_bool CCollider::Collision_OBB(const CCollider * pTargetCollider)
{
	...
	{
		//각 바운딩 박스에 평행한 축을 구한다.
		Compute_AlignAxis(&tOBB[i]);
		//각 바운딩 박스의 중심에서 각 면을 바라보는 벡터를 구한다.
	...
	for (size_t i = 0; i < 9; ++i)
	{
		...
		fDistance[1] = 
			fabs(D3DXVec3Dot(&tOBB[0].vProjAxis[0], &vAlignAxis[i])) +
			fabs(D3DXVec3Dot(&tOBB[0].vProjAxis[1], &vAlignAxis[i])) +
			fabs(D3DXVec3Dot(&tOBB[0].vProjAxis[2], &vAlignAxis[i]));

		fDistance[2] = 
			fabs(D3DXVec3Dot(&tOBB[1].vProjAxis[0], &vAlignAxis[i])) +
...
_bool CCollider::Collision_OBB(const CCollider* pTargetCollider, _vec3& vOut)
{
	const OBB* pTargetOBB = pTargetCollider->Get_OBBDescPointer();
	if (nullptr == pTargetOBB)
		return false;
