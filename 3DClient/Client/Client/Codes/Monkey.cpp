...
HRESULT CMonkey::Ready_Prototype()
{
	return S_OK;
}
...
HRESULT CMonkey::Ready_GameObject(void * pArg)
{
	...
	m_pMeshCom->Set_AnimationSet((_int)STATE_::STATE_IDLE1);
	m_pMeshCom_Shadow->Set_AnimationSet((_int)STATE_::STATE_IDLE1);


	m_pTransformCom->Scaling(0.1f, 0.1f, 0.1f);
	m_pTransformCom->SetUp_Speed(6.f, D3DXToRadian(360.f));
...
_int CMonkey::LastUpdate_GameObject(const _float & fTimeDelta)
{
...
void CMonkey::Render_GameObject()
{
...
void CMonkey::Render_GameObject_Shadow()
{
...
void CMonkey::Change_State(const _float& fTimeDelta)
{
	{
	...
	if (m_ePreState != m_eCurState)
	{
		switch (m_eCurState)
		{
		case STATE_::STATE_IDLE1:
			m_pMeshCom->Set_AnimationSet((_int)STATE_::STATE_IDLE1);
			m_pMeshCom_Shadow->Set_AnimationSet((_int)STATE_::STATE_IDLE1);
			break;
		case STATE_::STATE_IDLE2:
			m_pMeshCom->Set_AnimationSet((_int)STATE_::STATE_IDLE2);
			m_pMeshCom_Shadow->Set_AnimationSet((_int)STATE_::STATE_IDLE2);
			break;
		case STATE_::STATE_IDLE3:
			m_pMeshCom->Set_AnimationSet((_int)STATE_::STATE_IDLE3);
			m_pMeshCom_Shadow->Set_AnimationSet((_int)STATE_::STATE_IDLE3);
			break;
		case STATE_::STATE_WALK:
			...
			break;
		case STATE_::STATE_ATT:
			m_pMeshCom->Set_AnimationSet((_int)STATE_::STATE_ATT);
			m_pMeshCom_Shadow->Set_AnimationSet((_int)STATE_::STATE_ATT);
			break;
...
void CMonkey::End_Loop(const _float& fTimeDelta)
{
...
void CMonkey::Chase_Player(const _float& fTimeDelta)
{
...
void CMonkey::Attack1(const _float& fTimeDelta)
{
	m_isRotateEnd = false;
	m_IsOnce = false;
...
void CMonkey::MeaningLess_Move(const _float& fTimeDelta)
{
...
{
	m_IsOnce = false;
	m_eCurState = STATE_::STATE_IDLE1;
}

void CMonkey::IDLE2()
{
	m_IsOnce = false;
	m_eCurState = STATE_::STATE_IDLE2;
}

void CMonkey::IDLE3()
{
	m_IsOnce = false;
	m_eCurState = STATE_::STATE_IDLE3;
}

HRESULT CMonkey::Ready_Component()
{
	CManagement*		pManagement = CManagement::GetInstance();
	if (nullptr == pManagement)
		return E_FAIL;
	...
	// For.Com_Mesh
	m_pMeshCom = (CDynamic_Mesh*)pManagement->Clone_Component(SCENE_STATIC, L"Component_Mesh_Monkey");
	if (FAILED(Add_Component(L"Com_Mesh", m_pMeshCom)))
		return E_FAIL;
	m_pMeshCom_Shadow = (CDynamic_Mesh*)pManagement->Clone_Component(SCENE_STATIC, L"Component_Mesh_Monkey");
	...
	D3DXMatrixScaling(&matLocalTransform, 40.f, 40.f, 40.f);
	matLocalTransform._42 = 20.f;
	...
	D3DXMatrixScaling(&matLocalTransform, 40.f, 40.f, 150.f);
...
HRESULT CMonkey::SetUp_ConstantTable(LPD3DXEFFECT pEffect, const D3DXMESHCONTAINER_DERIVED * pMeshContainer, const _uint & iAttributeID)
{
...
HRESULT CMonkey::SetUp_ConstantTable_Shadow(LPD3DXEFFECT pEffect, const D3DXMESHCONTAINER_DERIVED* pMeshContainer, const _uint& iAttributeID)
{
	m_pTransformCom->SetUp_OnShader(pEffect, "g_matWorld");
