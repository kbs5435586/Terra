...
#include "Target.h"
CMeteor_Stone::CMeteor_Stone(LPDIRECT3DDEVICE9 pGraphic_Device)
    : CGameObject(pGraphic_Device)
{
}

CMeteor_Stone::CMeteor_Stone(const CMeteor_Stone& rhs)
    : CGameObject(rhs)
{
}

HRESULT CMeteor_Stone::Ready_Prototype()
{
    return S_OK;
}

HRESULT CMeteor_Stone::Ready_GameObject(void* pArg)
{
    {
        //if (nullptr == pArg)
        //    return E_FAIL;
 ...
 //     SCENE_STATIC, L"Layer_Fire_Tall", m_pTransformCom)))
 //     return E_FAIL;
...
_int CMeteor_Stone::Update_GameObject(const _float& fTimeDelta)
{
    ...
    if (m_fAccTime >= 0.1f)
    {
        if (FAILED(CManagement::GetInstance()->Add_GameObjectToLayer(L"GameObject_Particle_FireSparck",
            SCENE_STATIC, L"Layer_Particle_FireSpark", m_pTransformCom)))
            return E_FAIL;
   ...
   //m_pTransformCom->Go_ToTarget(&vPos, fTimeDelta);
    ...
    if (m_fLifeTime >= 1.4f)
    {
        //GameObject_Particle_Explosion
        if (FAILED(CManagement::GetInstance()->Add_GameObjectToLayer(L"GameObject_Particle_Explosion",
            SCENE_STATIC, L"Layer_Particle_Explosion", (m_pTransformCom->Get_StateInfo(STATE_POSITION)))))
            return E_FAIL;
        //GameObject_Effect_WaveRing
        if (FAILED(CManagement::GetInstance()->Add_GameObjectToLayer(L"GameObject_Effect_WaveRing",
            SCENE_STATIC, L"Layer_Effect_WaveRing", (m_pTransformCom->Get_StateInfo(STATE_POSITION)))))
            return E_FAIL;
        return DEAD_OBJ;
...
_int CMeteor_Stone::LastUpdate_GameObject(const _float& fTimeDelta)
{
...
void CMeteor_Stone::Render_GameObject()
{
...
void CMeteor_Stone::Render_GameObject_PostEffect()
{
...
CMeteor_Stone* CMeteor_Stone::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
    CMeteor_Stone* pInstance = new CMeteor_Stone(pGraphic_Device);
...
CGameObject* CMeteor_Stone::Clone_GameObject(void* pArg)
{
    CMeteor_Stone* pInstance = new CMeteor_Stone(*this);
...
HRESULT CMeteor_Stone::Ready_Component()
{
    ...
    m_pMeshCom = (CStatic_Mesh*)pManagement->Clone_Component(SCENE_STATIC, L"Component_EffectMesh_Meteor");
...
HRESULT CMeteor_Stone::SetUp_ConstantTable(LPD3DXEFFECT pEffect, const _uint& iAttributeID)
{
...
HRESULT CMeteor_Stone::SetUp_ConstantTable_PostEffect(LPD3DXEFFECT pEffect, const _uint& iAttributeID)
{
    m_pTransformCom->SetUp_OnShader(pEffect, "g_matWorld");
