...
void CDynamic_Mesh::Update_Skinning(const _uint& iMeshContainerIdx, vector<_matrix> pOut)
{
	if (m_vecMeshContainer.size() <= iMeshContainerIdx)
		return;
	...
	{
		pMeshContainer->pRenderingMatrices[i] = pMeshContainer->pOffsetMatrices[i] * *pMeshContainer->ppCombinedTransformationMatrices[i];
		pOut.push_back(pMeshContainer->pRenderingMatrices[i]);
	}
...
void CDynamic_Mesh::Update_Skinning_AnimBlur(const _uint& iMeshContainerIdx, _matrix* pIn)
{
	if (m_vecMeshContainer.size() <= iMeshContainerIdx)
		return;
	...
	// 메시를 뼈에 붙인다.
	pMeshContainer->pSkinInfo->UpdateSkinnedMesh(pIn, nullptr, pSour, pDest);
...
{
	m_iAnimIdx = iAnimationID;
	return m_pAnimationCtrl->Set_AnimationSet(m_iAnimIdx);
}
...
_uint CDynamic_Mesh::Get_TrackIndex()
{
	return m_pAnimationCtrl->Get_TrackIndex();
}

_double CDynamic_Mesh::Get_CurrTime()
{
	return m_pAnimationCtrl->Get_CurrTime();
}

_double CDynamic_Mesh::Get_AllTime()
{
	return m_pAnimationCtrl->Get_AllTime();
}
